...
(defn mapLabels [program]
  (loop [instrs program
    ...
    (cond (empty? instrs)
          ...
          (recur (rest instrs) (assoc labelMap (:lbl (first instrs))
                                      (second instrs)))
          :else
...
;; Note: This assumes that there are never two consecutive labels (a
;; situation which could be simplified to just one label anyhow).
;; Note: Even though LABELS are not real instructions, this defines
;; successors for them. They'll be ignored by liveness though.  The
;; last instruction in the program will be expected to map to nil,
;; since it has not successor.
(defn flow [program]
    ...
    (loop [instrs program
      ...
      (if (empty? instrs)
        ...
        (let [hasNext (not (isit? (first instrs) :minijava.gas/jmp))
              ...
              succMap (if hasNext
                        (addSuccessor succMap (first instrs) (second instrs))
                        succMap)
              succMap (if hasLabel
                        (addSuccessor succMap (first instrs)
                                      (get labelMap (:dst (first instrs))))
                        succMap)]
